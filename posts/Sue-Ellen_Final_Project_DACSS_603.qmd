...
Models 1 through 3 are all highly statistically significant for each coefficient and for the f-statistic. Models 4 and 5 demonstrate that the particular year of the accident has little to no statistical significance when regressed with other regressors. Regressed alone to the Number of Vehicles per Crash, Year has a high level of significance, but has a very small adjusted R2. 

In regards to adjusted R2 values, F-statistics and statistical significance, model 3 is the best fit. The model reads as follows:
...
In model 3, while holding I(AADT^2), I(AADT^3), SPEED_LIMIT, and Distracted_Binary constant, a 1 unit change in AADT results in a -0.00000396 change in the Number of Vehicles per Crash. Additionally, holding all other variables (AADT, I(AADT2)I(AADT2), I(AADT3)I(AADT3), SPEED_LIMIT) constant, the coefficient estimate for Distracted_Binary is 0.0959. This means that for every one-unit increase in the Distracted_Binary variable while keeping all other factors constant, the expected change in the Number of Vehicles per Crash is an increase of 0.0959. The adjusted R2 for this model is 0.04393736, which means that this model accounts for 4.4% of the variation in the response variable (Number of Vehicles per Crash). The remaining variability (95.6%) is attributed to other factors not considered in the model. 

What I had initially anticipated with this project was that the Number of Vehicles per Crash were increasing over time. I was only able to analyze the data over 10 years, and looking here at the sixth model we can see how small the adjusted R2 is for lm(NUMB_VEHC ~ YEAR), with Year only accounting for .004% of the variation in the Number of Vehicles per year. The F-statistic with a value of 16.19 and an associated p-value of 5.745e-05, indicate the regression is highly statistically significant, with strong evidence to suggest that the model as a whole has explanatory power and there is evidence of a relationship between Number of Vehicles per Crash and Year.

However, when regressed with the main regressor for this report, AADT (mod6), Year loses its significance as does the constant (Number of Vehicles per Crash). The F statistic still remains significant in mod6, but the loss of significance of the Year and Number of Vehicles per Crash lead me to believe Year is not the most beneficial regressor in considering Number of Vehicles per Crash without more extensive data. Continuing on this trend of discounting year as a beneficial regressor, when performing a linear hypothesis where Year is set to 0, the p-value increases to greater than the significance level of 0.05. In this context, its reasonable to conclude that Year does not have a substantial impact on the Number of Vehicles per Crash.


# Limitations

##### Breadth of Data
The comprehensive cleaning of data for such a large dataset present notable challenges within the constraints of a limited time frame. This is a project that could take years of parsing through the original annual Crash datasets produced by MASS DOT. The short list of data that could provide more insight into the exploration of the Number of Vehicles per Crash include: large text columns that categorize the contributing factors to a crash, individual demographic driver information, weather and road condition data and the massive field of geospatial data. The substantial depth and breadth of this dataset offer the potential for extracting robust analytical insights that extend beyond the surface-level information.
